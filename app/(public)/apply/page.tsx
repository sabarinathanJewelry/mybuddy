...
const sectionTitle = "text-base font-bold text-gray-800 pb-2 border-b border-gray-100";
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 max-w-md w-full text-center space-y-4">
          ...
          <p className="text-gray-500 text-sm leading-relaxed">
            Thank you for your interest in joining Sabarinathan Jewellers. We have received your application
            and will get back to you if your profile matches our requirements.
          </p>
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-1 pb-2">
          ...
          <p className="text-sm text-gray-500">Sales Executive — Candidate Information Form</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className={section}>
            ...
            <div>
              <label className={label}>1. Full Name <span className="text-red-500">*</span></label>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className={label}>3. Mobile Number <span className="text-red-500">*</span></label>
                <input className={inp} type="tel" value={form.mobile} onChange={e => set("mobile", e.target.value)} placeholder="10-digit number" />
              </div>
            </div>
            <div>
              <label className={label}>4. Current Address</label>
              <textarea className={textarea} rows={2} value={form.address} onChange={e => set("address", e.target.value)} placeholder="Your current residential address" />
            </div>
          </div>

          {/* 2. Current Employment */}
          <div className={section}>
            <p className={sectionTitle}>Current Employment</p>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={label}>5. Current Company</label>
                <input className={inp} value={form.current_company} onChange={e => set("current_company", e.target.value)} placeholder="Company name" />
              </div>
              <div>
                <label className={label}>6. Total Jewellery Experience</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={label}>7. Current Designation</label>
              ...
              <div>
                <label className={label}>8. Current Salary (Take Home)</label>
                <input className={inp} value={form.current_salary} onChange={e => set("current_salary", e.target.value)} placeholder="e.g. ₹18,000" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={label}>9. Incentive (if any)</label>
                <input className={inp} value={form.incentive} onChange={e => set("incentive", e.target.value)} placeholder="e.g. ₹2,000/month" />
              </div>
              <div>
                <label className={label}>10. Notice Period</label>
                <input className={inp} value={form.notice_period} onChange={e => set("notice_period", e.target.value)} placeholder="e.g. 1 month" />
              </div>
            </div>
            <div>
              <label className={label}>11. Reason for Leaving Current Job</label>
              <textarea className={textarea} rows={2} value={form.reason_leaving} onChange={e => set("reason_leaving", e.target.value)} placeholder="Be honest..." />
            </div>
          </div>

          {/* 3. Jewellery Knowledge */}
          <div className={section}>
            <p className={sectionTitle}>Jewellery Knowledge</p>

            <div>
              <label className={label}>12. Which section have you worked in?</label>
              <div className="grid grid-cols-2 gap-2 mt-1">
                {SECTIONS_LIST.map(s => (
                  <label key={s.key} className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={form.sections_worked.includes(s.key)}
            ...
            <div>
              <label className={label}>13. What are your daily responsibilities?</label>
              <textarea className={textarea} rows={3} value={form.daily_responsibilities} onChange={e => set("daily_responsibilities", e.target.value)} placeholder="Describe your day-to-day work..." />
            </div>
            <div>
              <label className={label}>14. What is your biggest achievement in your current job?</label>
              <textarea className={textarea} rows={2} value={form.biggest_achievement} onChange={e => set("biggest_achievement", e.target.value)} placeholder="Share something you are proud of..." />
            </div>
            <div>
              <label className={label}>15. What skills would you like to improve?</label>
              <textarea className={textarea} rows={2} value={form.skills_to_improve} onChange={e => set("skills_to_improve", e.target.value)} placeholder="Be honest about areas of growth..." />
            </div>
          </div>

          {/* 4. Customer Handling */}
          <div className={section}>
            <p className={sectionTitle}>Customer Handling</p>

            <div>
              <label className={label}>16. If a customer says our making charges are high, how would you convince them?</label>
              <textarea className={textarea} rows={3} value={form.handle_making_charges} onChange={e => set("handle_making_charges", e.target.value)} placeholder="Your approach..." />
            </div>
            <div>
              <label className={label}>17. How would you handle an angry customer?</label>
              <textarea className={textarea} rows={3} value={form.handle_angry_customer} onChange={e => set("handle_angry_customer", e.target.value)} placeholder="Your approach..." />
            </div>
            <div>
              <label className={label}>18. Have you ever handled old gold exchange? If yes, explain briefly.</label>
              <textarea className={textarea} rows={2} value={form.old_gold_experience} onChange={e => set("old_gold_experience", e.target.value)} placeholder="Yes / No — and details if yes..." />
            </div>
          </div>

          {/* 5. Salary & Career */}
          <div className={section}>
            <p className={sectionTitle}>Salary & Career</p>

            <div>
              <label className={label}>19. Expected Salary</label>
              <input className={inp} value={form.expected_salary} onChange={e => set("expected_salary", e.target.value)} placeholder="e.g. ₹22,000" />
            </div>
            <div>
              <label className={label}>20. Why do you think you deserve this salary?</label>
              <textarea className={textarea} rows={2} value={form.salary_justification} onChange={e => set("salary_justification", e.target.value)} placeholder="Your reasoning..." />
            </div>
            <div>
              <label className={label}>21. If your current employer increases your salary, will you stay there?</label>
              ...
              {form.stay_if_raised === "maybe" && (
                <div className="mt-2">
                  <label className={label}>Please explain:</label>
                  <textarea className={textarea} rows={2} value={form.stay_explanation} onChange={e => set("stay_explanation", e.target.value)} placeholder="Explain your answer..." />
                </div>
              )}
            </div>
            <div>
              <label className={label}>22. Where do you see yourself in the next 3 years?</label>
              <textarea className={textarea} rows={2} value={form.career_vision} onChange={e => set("career_vision", e.target.value)} placeholder="Your career goals..." />
            </div>
          </div>

          {/* 6. Integrity & Commitment */}
          <div className={section}>
            <p className={sectionTitle}>Integrity & Commitment</p>

            <div>
              <label className={label}>23. Have you ever received any warning or disciplinary action at work?</label>
            ...
            <div>
              <label className={label}>24. Are you willing to work during festivals, weekends and extended business hours?</label>
              <div className="flex gap-6 mt-1">
                {["yes", "no"].map(opt => (
                  <label key={opt} className="flex items-center gap-2 cursor-pointer select-none">
            ...
            <div>
              <label className={label}>25. Is there anything else you would like us to know about you?</label>
              <textarea className={textarea} rows={3} value={form.additional_info} onChange={e => set("additional_info", e.target.value)} placeholder="Optional..." />
            </div>
          </div>

          {/* Declaration */}
          <div className={section}>
            <p className={sectionTitle}>Declaration</p>
            ...
            <div className="grid grid-cols-2 gap-4 pt-1">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-gray-800 py-2">{new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}</p>
              </div>
            </div>
          </div>
          ...
          <p className="text-center text-xs text-gray-400 pb-4">
            Sabarinathan Jewellers · Confidential
          </p>
        </form>
      </div>
    </div>
