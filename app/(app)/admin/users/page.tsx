...
export default function AdminUsersPage() {
  ...
  const togglePhotoShootAccess = useMutation({
    mutationFn: async ({ id, value }: { id: string; value: boolean }) => {
      const { error } = await supabase().from("profiles").update({ photo_shoot_access: value }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["profiles"] }),
  });
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-canvas text-xs text-ink-dim border-b border-line">
                ...
                <th className="text-center px-3 py-2.5">Photo</th>
                    ...
                    <tr key={p.id} className={`border-b border-line last:border-0 hover:bg-canvas/50 ${p.is_active === false ? "opacity-50" : ""}`}>
                      ...
                      <td className="px-3 py-2.5 text-center">
                        {p.role === "admin" ? (
                          ...
                          <button
                            onClick={() => togglePhotoShootAccess.mutate({ id: p.id, value: !p.photo_shoot_access })}
                            className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                              p.photo_shoot_access
                                ? "bg-ok/10 border-ok/30 text-ok"
                                : "border-line text-ink-dim hover:border-gold hover:text-gold"
                            }`}
                          >
                            {p.photo_shoot_access ? "On" : "Off"}
                          </button>
                        )}
                      </td>
                      <td className="px-3 py-2.5 text-center">
