...
import { useSupplier360, useSaveSupplierPurchase, useSaveSupplierPayment } from "@/modules/suppliers/api";
...
export default function Supplier360Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const totalPurchased = view?.purchases.reduce((s, p) => s + (p.amount ?? 0), 0) ?? 0;
  const totalPaid = view?.payments.reduce((s, p) => s + (p.amount ?? 0), 0) ?? 0;
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      {/* Summary */}
      <div className="bg-white rounded-xl border border-line p-5 shadow-soft grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div><p className="text-xs text-ink-dim">{t("cash_balance")}</p><p className={`text-xl font-bold ${cashBalance > 0 ? "text-err" : "text-ok"}`}>{inr(cashBalance)}</p></div>
        <div><p className="text-xs text-ink-dim">Total Purchased</p><p className="text-xl font-bold text-ink">{inr(totalPurchased)}</p></div>
        <div><p className="text-xs text-ink-dim">Suspense Items</p><p className="text-xl font-bold text-warn">{view?.suspense.length ?? 0}</p></div>
      </div>
            ...
            <form onSubmit={handlePurchaseSave} className="bg-white border border-line rounded-xl p-4 shadow-soft space-y-3">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <div><label className="text-xs text-ink-dim">Metal</label>
                  <select value={purchaseForm.metal} onChange={(e) => setPurchaseForm({ ...purchaseForm, metal: e.target.value })}
        ...
        <div className="space-y-3">
          ...
          {showPaymentForm && (
            <form onSubmit={handlePaymentSave} className="bg-white border border-line rounded-xl p-4 shadow-soft space-y-3">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div><label className="text-xs text-ink-dim">Date</label>
                  <input type="date" value={paymentForm.pay_date} onChange={(e) => setPaymentForm({ ...paymentForm, pay_date: e.target.value })}
                    className="w-full border border-line rounded-lg2 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gold" /></div>
                ...
                {(paymentForm.mode === "old_gold" || paymentForm.mode === "old_silver") && (
                  <>
                    <div><label className="text-xs text-ink-dim">Metal Wt</label>
                      <input type="number" step="0.001" value={paymentForm.metal_wt} onChange={(e) => setPaymentForm({ ...paymentForm, metal_wt: parseFloat(e.target.value) || 0 })}
                        className="w-full border border-line rounded-lg2 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gold" /></div>
                    <div><label className="text-xs text-ink-dim">Purity%</label>
                      <input type="number" step="0.01" value={paymentForm.metal_purity} onChange={(e) => setPaymentForm({ ...paymentForm, metal_purity: parseFloat(e.target.value) || 0 })}
                        className="w-full border border-line rounded-lg2 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gold" /></div>
                  </>
                )}
                {paymentForm.mode === "cut_rate" && (
                  <div><label className="text-xs text-ink-dim">Cut Rate/g</label>
                    <input type="number" step="0.01" value={paymentForm.cut_rate} onChange={(e) => setPaymentForm({ ...paymentForm, cut_rate: parseFloat(e.target.value) || 0 })}
                      className="w-full border border-line rounded-lg2 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-gold" /></div>
                )}
              </div>
              <div className="flex gap-2">
            ...
            <tbody>
              {view?.suspense.map((s) => (
                <tr key={s.id} className="border-b border-line last:border-0 hover:bg-canvas/50">
                  ...
                  <td className="px-3 py-2.5 text-right">{grams(s.pure_wt ?? 0)}</td>
                  <td className="px-3 py-2.5 text-right font-mono">{inr(s.line_total ?? 0)}</td>
                </tr>
              ))}
              {!view?.suspense.length && <tr><td colSpan={5} className="px-4 py-6 text-center text-ink-dim">{t("no_data")}</td></tr>}
            </tbody>
          </table>
        </div>
      )}
