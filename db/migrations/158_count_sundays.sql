-- 158: Count all 7 days for punctuality and break discipline.
-- The shop works on Sundays, so Sunday punches should count the same as any other day.
-- Previously Mon-Sat only (isodow BETWEEN 1 AND 6), now all days they punch in.
...
BEGIN
  ...
  -- All days punched in count — shop is open 7 days a week
  punctuality AS (
  ...
  -- Break discipline: all days they punch in (including Sunday)
  break_days AS (
    SELECT dp.bio_user_id,
      count(*) FILTER (
