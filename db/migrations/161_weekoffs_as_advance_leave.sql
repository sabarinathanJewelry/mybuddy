-- 161: Treat planned week-offs as advance-notice leaves in reward scoring.
--
-- monthly_weekoffs stores staff-planned days off (approved/pending).
-- Previously these appeared as "absent without request" → timing 2 → 5/10.
-- Now: if an absent day matches a planned week-off → timing 0 (advance) → 10/10.
-- Unplanned absences (no week-off, no leave_request) still default to timing 2.
...
BEGIN
  ...
  -- Absent days: calendar days in month with NO punch record
  absent_days AS (
  ...
  -- Formal leave requests with timing
  leave_reqs AS (
  ...
  -- Planned week-offs submitted through the app = advance notice (timing 0)
  weekoff_days AS (
  ...
  -- For each absent day: week-off = timing 0, leave_request timing if present, else 2
  leaves AS (
    SELECT
      ad.bio_user_id,
