-- 160: Count actual absent days (no punch) as leaves for reward scoring.
--
-- Previously only formal leave_requests were counted — staff who just didn't
-- show up (like SIVASANKAR with 10 absent days) incorrectly scored 10/10.
--
-- New approach:
--   leave_count  = days with NO attendance punch in the month
--   worst_timing = based on formal leave_request if one exists for that day;
--                  absent WITHOUT any request → timing 2 (worst, like same-day)
--
-- This matches the salary module's absent-day calculation.
...
BEGIN
  ...
  -- Absent days: calendar days in month with NO punch record for that staff member
  absent_days AS (
  ...
  -- Formal leave requests (for timing info)
  leave_reqs AS (
  ...
  -- For each absent day: if a formal leave_request exists use its timing,
  -- otherwise treat as worst-case (no advance notice = timing 2)
  leaves AS (
    SELECT
      ad.bio_user_id,
      count(*)::int                        AS leave_count,
      max(coalesce(lr.timing, 2))          AS worst_timing
    FROM absent_days ad
  ...
  SELECT
    ...
    -- Leave score based on actual absent days + timing
    CASE
