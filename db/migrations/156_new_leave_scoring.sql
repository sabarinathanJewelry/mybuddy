-- 156: New leave scoring based on how early the leave was applied.
--
-- Scoring (worst-case per month wins):
--   All leaves applied in advance (created_at < leave_date) → 10 pts
--   At least one retrospective leave (created_at > leave_date) → 8 pts
--   At least one same-day leave (created_at::date = leave_date)  → 5 pts
--   More than 3 approved leaves → leave_flag = true (management review)
--
-- Week-off scoring is deferred — monthly_weekoffs uses profiles.user_id
-- which has no FK to staff.bio_user_id; will require a mapping column.

DROP FUNCTION IF EXISTS calculate_monthly_rewards(text);
DROP FUNCTION IF EXISTS refresh_monthly_rewards(text);

-- Add leave_flag column if not present
ALTER TABLE monthly_reward_scores
  ADD COLUMN IF NOT EXISTS leave_flag boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  SELECT
    ...
    -- Leave score: worst-case timing determines the score
    CASE
      WHEN coalesce(l.worst_timing, 0) = 2 THEN 5   -- same-day application
      WHEN coalesce(l.worst_timing, 0) = 1 THEN 8   -- retrospective (applied after leave date)
      ELSE 10                                         -- all in advance or no leaves
    END AS leave_pts,
    ...
    -- Flag if more than 3 approved leaves (management review)
