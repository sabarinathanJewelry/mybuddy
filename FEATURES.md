...
- **Manual cutoff date for arrear**: in the mapping dialog (arrear mode), an "Exclude bills paid on/before" date picker appears; set this to the date the regular incentive was processed (e.g. 2026-07-03) and all bills with `paidDate ≤ that date` are excluded from arrear — overrides the automatic lock-date filter; essential for sheets that were locked before `lockedAt` was stored
