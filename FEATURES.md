...
- **"Paid On" date in arrear audit**: both the mapping dialog bill expansion and the payroll audit modal show a "Paid On" column (green, YYYY-MM-DD) — the date admin clicked "Paid ✓" in the incentive calculator; if this date is before the export date (e.g. before July 4 for a June sheet), the bill was likely already included in the regular incentive and should NOT be loaded as arrear to avoid double payment
