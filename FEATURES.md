...
- Per-supplier table: Period Purchases (₹) · Gold In (g) · Period Payments (₹) · Outstanding (₹) as of period end
- Outstanding = opening_balance + all purchases to date − all payments to date (running balance, not period-only)
- Filters: FY year selector + period buttons (Full FY / Q1 Apr–Jun / Q2 Jul–Sep / Q3 Oct–Dec / Q4 Jan–Mar / Month)
- Summary cards: total purchases, payments, gold purchased, total outstanding for the period
- Supplier names link to their detail page; CR suffix when supplier has a credit balance
- Linked from Suppliers list page header ("Outstanding" button)
...
- Chit payment edit: editable date, amount, and mode; date change also updates `tx_date` in `cash_ledger`/`bank_ledger`; recalculates grams using stored board_rate; disabled for "Split" entries
...
- **Inventory Movement section**: opening stock defaults — Gold from the Gold Stock section's latest entry on/before the period start date (gross 22K weight × 91.67% → pure gold); Silver from Kolusu boxes reconstructed as of the period start (gross weight × 65% → pure silver at 65 touch); either can be overridden and saved as a dedicated `metal_inventory_snapshots` row per period. Then + in (purchases + old metal + exchange) − out (sold + dispatched + bullion sold) = closing stock estimate; closing value at WAC v2
...
- **FY Monthly Touch Table**: comprehensive sold touch% vs purchase touch% per month for gold and silver separately; FY year selector; gross weight column for each metal so volumes are transparent
  - Sold touch = back-calculated from actual sale price: `metal_revenue = (line_total ÷ (1 + GST%)) − making − stone/diamond`; `VA% = (metal_revenue / (gross_wt × board_rate) − 1) × 100`; `touch = base_purity + VA%`. Formula: VA is the % above board rate (excl GST), touch adds that to the physical purity baseline so 22K at 10% VA = 101.67% touch. Falls back to stored `purity_pct + va_pct` when rate/line_total missing
  - Avg VA% column uses `touch − base_purity` (price-derived) not the stored `va_pct` field
...
- **Payment history**: click the "Cash Paid" amount on any trade row to expand an inline payment list showing each instalment's date, amount, and mode (sorted by date); each payment has an Edit button to correct its date or amount in-place (also updates the matching cash_ledger/bank_ledger tx_date)
...
- Daily attendance view: all staff with punch times, late flag, hours worked; **expand a row → Edit Punches panel lets admin edit the time of any individual punch, delete it, or add a new punch** (e.g. remove an accidental double-out punch or restore a missing OUT punch); changes refresh the row immediately
- Monthly attendance summary: present days, late days, OT, leaves, deductions; **click any date in the per-staff day rows to expand a Punch Log panel** showing all raw punches with Edit/Delete per punch and an Add Punch button — same controls as the daily view, changes invalidate both the daily and monthly caches
- **Admin punch overrides are sync-safe**: deleted and edited punches are soft-deleted (`deleted_by_admin=true`) so the biometric device sync (upsert on `bio_user_id,punch_time`) can never restore them — the conflict row stays flagged and all queries filter it out (migration 147)
- **Salary increment history**: when admin saves a salary change from the monthly attendance edit panel, the old → new amount and effective month are recorded in `staff_salary_history`; duplicate entries (same old→new→month) are filtered out in the display; saving now correctly refreshes the monthly summary so re-saving doesn't create false duplicates; history is shown inline below the edit form (date changed, old salary in red, new salary in green, effective month) — only future edits are tracked, past changes before this feature are not recorded
...
- **Incentive rate override per row**: Inc cell now shows the master ₹/g rate inline; click to override the rate for that specific row only (shown in blue, strikethrough of master rate, ↩ to reset) — useful for low-wastage rows where a partial incentive is warranted
- **Diamond items use per-sale (flat) incentive**: DIAMOND BESARI (₹100), DIAMOND STUD (₹200), DIAMOND RING (₹200), DIAMOND NECKLACE (₹500) are calculated as a flat amount per bill — not multiplied by net weight; rate label shows `/sale` instead of `/g`; `DIA NECKLACE` ERP name maps to DIAMOND NECKLACE
- **Kolusu restock full edit**: "Edit" button on restock history rows opens a form to change date, qty, gross weight, and notes; box stock (`current_qty`, `current_gross_wt_g`) is adjusted by the diff so totals stay accurate
...
- **Partial-month salary prorating**: per-day salary always uses `monthly_salary ÷ 30` (standard payroll divisor); for staff who joined during the current month, net pay is prorated as `days_in_period × per_day_salary` instead of the full monthly amount; split-up card shows "÷ 30 days × N in period" when prorated
- **Conduct & Contact card in payroll**: each expanded staff row now shows a "Contact & Conduct" card alongside the Salary Split-up card — displays the staff's phone number (clickable tel: link), a breakdown of conduct issues this month (late arrivals, excess absences, double punches), a total issue count, and a "Show Warning" button that generates a warning notice ("₹10 per issue will be deducted next month if this continues"); table Name column shows a red "N issues" badge for staff with any conduct issues
...
- **Two separate nav surfaces, both needed**: `sidebar.tsx` (used by `/admin/*` and full-login users under `app/(app)/layout.tsx`) is a completely different UI from the staff smart-home app (`app/(staff)/my-attendance/page.tsx`, its own route group) — a staff-role login never sees `sidebar.tsx` at all. The access flag had to be wired into both: `sidebar.tsx` for admin/subadmin/signage logins, and `my-attendance/page.tsx`'s header link + Smart View "Shop Tools" card (mirroring exactly how `repair_access`/`kolusu_access` are surfaced there) for staff-role logins
...
- **`staff` + `attendance_logs` RLS fallback via `user_id`** (`db/migrations/140_staff_rls_user_id_fallback.sql`): the original RLS matched staff to their own row via `bio_user_id = auth.jwt()->'app_metadata'->>'bio_user_id'`, which requires the JWT to have been issued after `app_metadata` was set. Added `OR user_id = auth.uid()` to `staff_select` and `OR bio_user_id IN (SELECT bio_user_id FROM staff WHERE user_id = auth.uid())` to `logs_select` so attendance works as long as `staff.user_id` is linked, regardless of JWT freshness. To link a staff login: `UPDATE staff SET user_id = '<auth_user_uuid>' WHERE bio_user_id = '<bio_id>'`
...
- **Arrear source label**: arrear column shows the source period (e.g. "July 2026 inc.") below the amount; payslip line reads "Arrear (July 2026 Incentive)" for clarity
- **Already applied badge**: incentive sheet picker shows an orange "Already applied" badge on sheets already loaded into this payroll period; mapping dialog repeats the warning so loading a second time is a deliberate choice
- **Arrear audit trail**: in the mapping dialog (arrear mode), each staff row shows a "▼ N bills" expand button; clicking opens a bill-level table showing date, bill no, customer name, product, and incentive amount for every `balanceZero`-marked row contributing to that staff member's arrear; bill metadata (date, customer, bill no) is carried forward per ERP group so multi-item bills show correctly
- **Arrear bills stored on entry**: after applying, each payroll entry with arrear stores the full bill list (`arrearBills`); the Arrear cell shows a "N bills" button that opens a full audit modal with a scrollable table and total footer — lets admin verify the source bills any time, even after the mapping dialog is closed
- **"Paid On" date in arrear audit**: both the mapping dialog bill expansion and the payroll audit modal show a "Paid On" column (green, YYYY-MM-DD) — the date admin clicked "Paid ✓" in the incentive calculator
- **Lock-date arrear filter (double-payment prevention)**: when "Lock" is clicked for a staff member, the lock date (`lockedAt`) is stored on each locked row; arrear calculation skips any bill where `paidDate ≤ lockedAt` for that staff — meaning if a bill was marked paid on the same day or before the regular incentive was locked, it was already included in that month's payment and must not appear as arrear; bills marked paid after the lock date are legitimate arrear and are included
- **Auto-detect cutoff date**: when "Load as Arrear" is first checked, the system scans all `balanceZero` overrides, finds the earliest `paidDate`, and pre-fills the "Exclude bills paid on/before" input with that date — so bills already processed in the regular incentive are automatically hidden without the user having to type anything; the date is editable and clearable; resets when a different incentive sheet is selected
- **Diamond perSale in payroll calc**: payroll's `calcStaffIncentives` now respects the `perSale` flag on master entries — diamond items (DIAMOND NECKLACE, DIA NECKLACE, etc.) calculate as a flat rate per sale rather than rate × net weight, matching the incentive-calc page behaviour
- **Incentive sheet overrides respected in payroll**: `calcStaffIncentives`, `getEligibleRowsForStaff`, and `getArrearBillDetails` now honour all per-row overrides saved in the incentive sheet — `rateOverride` (per-row rate correction set in the Inc cell) and `forceIneligible` (rows skipped with "↓ skip"); previously these were ignored so payroll totals could differ from what was shown in the incentive calculator
...
- **CMS pages** (`app/(app)/admin/signage/{playlists,channels,devices}/page.tsx`, shared tab strip via `src/components/signage/signage-tabs.tsx`):
  - **Playlists**: create playlists, add items by uploading an image or video (into a public `signage-media` bucket) with a duration in seconds, drag-to-reorder via `@dnd-kit`. Images over 50MB are compressed client-side (`compressImage()`, 2560px/85% quality — TV-resolution, not the 900px preset used for repair-photo thumbnails) before upload rather than rejected. Deleting a playlist item also removes its file from `signage-media` (previously only deleted the DB row, leaving orphaned files in storage)
  - **Video sound**: `<video>` autoplay was `muted` by default (browsers block unmuted autoplay without a prior user gesture, and there's no such gesture on a remote-only TV). Fixed at the native layer instead — `signage-tv-app/.../MainActivity.java` calls `getBridge().getWebView().getSettings().setMediaPlaybackRequiresUserGesture(false)`, since this is our own controlled kiosk shell. Only takes effect in the packaged APK, not a plain browser tab (which still blocks it per normal browser policy)
...
- **`app/tv-player/page.tsx`** (built): the actual player — pairing-code screen while unclaimed, then renders each zone as an absolutely-positioned percentage rectangle looping its resolved items (images on a timer, videos advance on `onEnded`); uses `object-fit: contain` (not `cover`) so uploaded images/videos always show in full rather than being cropped to fill a zone whose aspect ratio doesn't match the source image. For zero letterboxing, source images should be exported at the zone's actual aspect ratio — for a vertical split, a zone that's X% of the screen width has ratio `(X/100 × 16) : 9` on a standard 16:9 TV. **CDN egress fix**: all playlist items are now rendered simultaneously with CSS `display:none` on inactive ones (keyed by position index, not URL) — the previous `key={media_url}` approach caused React to unmount+remount every img/video on each slide transition, re-downloading the file from Supabase CDN on every rotation and causing 79 GB of excess egress in a single billing cycle. Files now download once per TV session, dramatically cutting Storage CDN usage
- **Supabase egress advisory panel** on the Playlists page: explains the 5 GB/month free-plan limit, compression guidelines (images < 500 KB, videos < 10 MB H.264), and links to the Supabase billing dashboard
- **Screensaver prevention**: the web-level `useScreenWakeLock()` (`navigator.wakeLock`, in `app/tv-player/page.tsx`) turned out unreliable on real Android TV hardware — the OS screensaver ("Daydream") watches for remote-control input, not whether a page inside the WebView reports itself active. The real fix is native: `signage-tv-app/android/app/src/main/java/.../MainActivity.java` sets `WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON` in `onCreate()`, the OS-level signal that actually keeps the screen on. This requires an APK rebuild + reinstall to take effect (unlike the pure-web fixes elsewhere in signage, this one is baked into the installed app itself)
...
- **Signage-only login** (`db/migrations/136_signage_role.sql`): new `profiles.role = 'signage'` value, same pattern as `078_subadmin_role.sql` adding `subadmin`. Hard-enforced in `middleware.ts` (not just UI hiding) — locked to `/admin/signage/*` and nothing else in the ERP, redirected there on login, exempt from MFA (same reasoning as staff). `sidebar.tsx` shows just one "Signage" link for this role instead of the full nav. Assign it from `/admin/users` — the new "Signage" column has a "Restrict" button (uses the same `/api/admin/set-role` route as the existing Sub-Admin toggle, now also accepting `"signage"`). The login itself still has to be provisioned the normal way (Supabase Dashboard → Authentication → Users → Add user) since there's no in-app "create user" flow — the new profile defaults to `staff`, then `/admin/users` restricts it to `signage`
- **Kiosk-lock exemption fix**: `KioskProvider` (wraps every `(app)` page) auto-locks to the shared attendance kiosk board (`/attendance`) after 5 minutes idle, and only exempted `staff`/restricted-`subadmin` roles — not the new `signage` role. Since `/attendance` isn't under `/admin/signage/*`, middleware bounced a locked signage login away from it, `KioskProvider` immediately retried, creating an infinite redirect loop that looked like every page bounced back to Playlists. Fixed by exempting `role === "signage"` from both the initial lock state and the idle-lock timer in `src/components/layout/kiosk-provider.tsx`

---

## Staff Task Reminders (Cron)

Daily push notifications for staff tasks, fired at 9 AM IST (3:30 AM UTC) via Vercel Cron.
...
- No new migration needed — uses existing `staff_tasks`, `staff`, `web_push_subscriptions`, and `profiles` tables

---

## Photo Shoot Tracker (`/photo-shoot`)

Tracks showcase items taken out for product photography and returned, with pipeline planning for upcoming shoots.

- **Sections**: configurable product categories (Antique Haram, Gold Haram, Necklace, Bangles, Earrings, Chains, Rings, Pendant, Other) — admin can add/rename/hide via the Sections tab
