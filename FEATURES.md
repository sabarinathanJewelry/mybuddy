...
- **Print All Stock**: "அச்சிடு / Print" button in header opens a full bilingual (Tamil/English) stock report — shop header, grand total summary, Vault table (category, tagged weight, qty, bulk, reserved) and Outer table, signature lines; uses `position: absolute` + `@page` margins so content flows across multiple A4 pages correctly; table headers repeat on each page via `display: table-header-group`
